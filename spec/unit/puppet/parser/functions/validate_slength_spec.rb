...
describe "the validate_slength function" do
  ...
  it "should raise a ParseError if argument 3 converted, but to 0, e.g. a string" do
    expect { scope.function_validate_slength(["moo",2,"monkey"]) }.to(raise_error(Puppet::ParseError, /please pass a positive number as min_length/))
  end

  it "should raise a ParseError if argument 3 converted, but to 0" do
    expect { scope.function_validate_slength(["moo",2,"0"]) }.to(raise_error(Puppet::ParseError, /please pass a positive number as min_length/))
  end
